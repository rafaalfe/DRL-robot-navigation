 ...
 *
 * Node ini mengambil tiga "irisan" horizontal (atas, tengah, bawah) dari gambar kedalaman,
 * menggunakan parameter kalibrasi kamera untuk melakukan deproyeksi menjadi titik-titik 3D (X, Y, Z),
 * dan menghasilkan pesan PointCloud2 yang terstruktur (height=3, width=num_rays).
 */
...
private:
    ...
    int num_rays_;
    double angle_min_; // PERBAIKAN: Variabel yang hilang ditambahkan kembali
    double angle_max_; // PERBAIKAN: Variabel yang hilang ditambahkan kembali
...
public:
    DepthToOrderedPointCloud() : nh_("~"), intrinsics_received_(false)
    {
        ...
        nh_.param("frame_id", frame_id_, std::string("camera_l515_depth_optical_camera"));
        
        // PERBAIKAN: Membaca parameter sudut yang hilang
        ...
        ROS_INFO("PointCloud density set to 3x%d points.", num_rays_);
    }
    ...
    void infoCallback(const sensor_msgs::CameraInfo::ConstPtr& info_msg)
    {
        ...
        ROS_INFO("Scan heights set to: Top=%d, Middle=%d, Bottom=%d", 
                 slice_height_top_, slice_height_middle_, slice_height_bottom_);
    }
    ...
    void depthCallback(const sensor_msgs::Image::ConstPtr& depth_msg)
    {
        ...
        try {
            ...
            cloud_msg.height = 3; // Tiga baris untuk pindaian atas, tengah, bawah
            cloud_msg.width  = num_rays_;
            cloud_msg.is_dense = false; // Karena mungkin ada titik tidak valid (NaN)
            cloud_msg.is_bigendian = false;
            
            sensor_msgs::PointCloud2Modifier modifier(cloud_msg);
            modifier.setPointCloud2FieldsByString(1, "xyz");
            
            // Iterator untuk mengisi data
            sensor_msgs::PointCloud2Iterator<float> iter_x(cloud_msg, "x");
            sensor_msgs::PointCloud2Iterator<float> iter_y(cloud_msg, "y");
            sensor_msgs::PointCloud2Iterator<float> iter_z(cloud_msg, "z");

            std::vector<int> slice_rows = {slice_height_top_, slice_height_middle_, slice_height_bottom_};
            // PERBAIKAN: Variabel angle_... sekarang sudah dideklarasikan dan bisa diakses
            double angle_increment = (angle_max_ - angle_min_) / (num_rays_ - 1);

            // Iterasi untuk setiap baris pindaian (atas, tengah, bawah)
            for (int slice_idx = 0; slice_idx < 3; ++slice_idx) {
                ...
                // Iterasi untuk setiap kolom (sinar)
                for (int ray_idx = 0; ray_idx < num_rays_; ++ray_idx) {
                    ...
                    if (target_col < 0 || target_col >= image_width_) {
                        // Jika di luar gambar, isi dengan NaN dan lanjutkan
                    ...
                    if (!valid_depths.empty()) {
                        ...
                        if (depth_z_meters >= range_min_ && depth_z_meters <= range_max_) {
                            // Hitung koordinat X, Y, Z dunia nyata
                            ...
                            // Isi data point cloud pada indeks yang benar
